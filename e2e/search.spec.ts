...
test.describe('Law Search', () => {
  ...
  test('should display search form with three tabs', async ({ page }) => {
    ...
    await expect(page.getByRole('tab', { name: '法令名' })).toHaveAttribute('aria-selected', 'true');
  });
  ...
  test('should search by law name', async ({ page }) => {
    ...
    await page.waitForSelector('text=/検索結果|エラー|見つかりませんでした/', { timeout: 10000 });
  });
  ...
  test('should switch to keyword search tab', async ({ page }) => {
    ...
    await expect(page.getByPlaceholder('検索語句を入力してください。')).toBeVisible();
  ...
  test('should convert numbers to kanji in law number fields', async ({ page }) => {
  ...
  test('should show/hide date picker based on law time selection', async ({ page }) => {
  ...
  test('should expand and collapse law types section', async ({ page }) => {
    ...
    const summaryText = await page.locator('text="全て選択中"').first().isVisible();
  ...
  test('should have collapsible sections for law types and categories', async ({ page }) => {
    // Wait for the form to load
    await page.waitForTimeout(1000);
  ...
  test('should handle search with no results', async ({ page }) => {
    ...
    await page.getByPlaceholder('法令名を入力してください。').fill('存在しない法令名XXXXXX');
    await page.getByRole('button', { name: '検索' }).click();
    ...
    const hasNoResults = await page.getByText(/検索結果が見つかりませんでした|見つかりませんでした|0件/).count() > 0;
