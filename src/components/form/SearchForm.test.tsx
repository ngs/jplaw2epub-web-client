...
describe('SearchForm', () => {
  ...
  it('should show law name search by default', () => {
    ...
    expect(screen.getByPlaceholderText('法令名を入力してください。')).toBeInTheDocument();
  });
  ...
  it('should switch to keyword search when keyword tab is clicked', () => {
    ...
    expect(screen.getByPlaceholderText('検索語句を入力してください。')).toBeInTheDocument();
  });
  ...
  it('should apply initial values when provided', () => {
    ...
    const input = screen.getByPlaceholderText('法令名を入力してください。') as HTMLInputElement;
